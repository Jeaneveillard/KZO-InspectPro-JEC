...
- "conforme" : aucune anomalie visible`;
    ...
    // ============================================================
    // SYSTÈME 3 AGENTS IA EN CHAÎNE (REIBH 2024 Pipeline)
    // ============================================================

    /**
     * Moteur universel d'appel API multi-provider.
     * @param {string} systemPrompt  - Prompt système
     * @param {string|Object} userContent - Texte OU { text, imageBase64, mediaType }
     * @param {string} agentKey      - 'agent1' | 'agent2' | 'agent3'
     * @returns {Promise<string>}    - Réponse brute du LLM
     */
    /**
     * Extrait et parse un objet JSON depuis une réponse LLM brute.
     * Résistant aux fences markdown (```json...```) et au texte parasite avant/après.
     * @param {string} raw - Texte brut renvoyé par le LLM
     * @returns {Object}   - Objet JSON parsé
     */
    _extractJSON: function(raw) {
        // Étape 1 : retirer les fences markdown (```json...``` ou ```...```)
        let cleaned = raw.replace(/^```(?:json)?\s*/im, '').replace(/```\s*$/im, '').trim();

        // Étape 2 : tenter de trouver le premier bloc JSON équilibré {…}
        ...
        if (end === -1) throw new Error('JSON mal formé — accolade fermante introuvable.');

        const jsonStr = cleaned.slice(start, end + 1);
        try {
            return JSON.parse(jsonStr);
        } catch(e) {
            throw new Error('Impossible de parser le JSON extrait : ' + e.message + '\n→ ' + jsonStr.slice(0, 200));
        }
    },

    _callAI: async function(systemPrompt, userContent, agentKey) {
        // Résolution provider : per-agent → global → KZO_CONFIG
        const provider = localStorage.getItem('inspectpro_api_provider_' + agentKey)
                      || localStorage.getItem('inspectpro_api_provider')
                      || (typeof KZO_CONFIG !== 'undefined' ? KZO_CONFIG.provider : 'gemini');
        const apiKey   = localStorage.getItem('inspectpro_api_key_' + agentKey)
                      || localStorage.getItem('inspectpro_api_key')
                      || (typeof KZO_CONFIG !== 'undefined' ? KZO_CONFIG.apiKey : null);

        if (!apiKey) {
            throw new Error('Aucune clé API configurée pour ' + agentKey + '. Veuillez la configurer dans les paramètres (⚙️).');
        }

        // Séparer texte et image
        const hasImage  = userContent && typeof userContent === 'object' && userContent.imageBase64;
        const textPart  = hasImage
                        ? (userContent.text || '')
                        : (typeof userContent === 'string' ? userContent : JSON.stringify(userContent));
        const imgBase64 = hasImage ? userContent.imageBase64 : null;
        const mimeType  = hasImage ? (userContent.mediaType || 'image/jpeg') : null;

        // Groq ne supporte pas la vision
        if (provider === 'groq' && hasImage) {
            throw new Error(
                'Le provider Groq ne supporte pas l\'analyse d\'images (vision). ' +
                'Veuillez sélectionner Anthropic, Gemini ou OpenAI pour l\'Agent 1.'
            );
        }
        ...
        if (provider === 'anthropic') {
            const model = hasImage ? 'claude-haiku-4-5-20251001' : 'claude-sonnet-4-5';
            const msgContent = hasImage
                ? [
            ...
            if (!resp.ok) {
                ...
                throw new Error('Erreur API Anthropic : ' + (e.error?.message || resp.status));
            }
        ...
        // ── GEMINI ─────────────────────────────────────────────────
            ...
            } catch(e) { /* garder le modèle par défaut */ }
            ...
            if (!resp.ok) {
                const e = await resp.json();
                throw new Error('Erreur API Gemini : ' + (e.error?.message || resp.status));
            }
            const data = await resp.json();
        ...
        // ── OPENAI ─────────────────────────────────────────────────
            ...
            if (!resp.ok) {
                const e = await resp.json();
                throw new Error('Erreur API OpenAI : ' + (e.error?.message || resp.status));
            }
            const data = await resp.json();
        ...
        // ── GROQ (texte seulement) ─────────────────────────────────
            ...
            if (!resp.ok) {
                const e = await resp.json();
                throw new Error('Erreur API Groq : ' + (e.error?.message || resp.status));
            }
            const data = await resp.json();
            rawText = data.choices?.[0]?.message?.content || '';

        } else {
            throw new Error('Provider inconnu : ' + provider + '. Providers supportés : anthropic, gemini, openai, groq.');
        }
    ...
    /**
     * Pipeline séquentiel 3 agents IA.
     * @param {string}   imageBase64  - Image en base64 (sans préfixe data:...)
     * @param {string}   mediaType    - MIME type (ex: 'image/jpeg')
     * @param {string}   sectionName  - Nom de la section d'inspection
     * @param {Function} onProgress   - Callback(etape, message)
     * @returns {Promise<Object>}     - { texteRapport, defauts, evaluations, niveauGlobal, recommandationPrincipale }
     */
    runPipeline3Agents: async function(imageBase64, mediaType, sectionName, onProgress) {
        const progress = (typeof onProgress === 'function') ? onProgress : function() {};

        // ──────────────────────────────────────────────────────────
        // AGENT 1 — Détecteur Visuel
        // ──────────────────────────────────────────────────────────
        progress('agent1', 'Agent 1 en cours — Analyse visuelle des défauts...');

        const systemAgent1 = [
            'Tu es un expert en inspection visuelle de bâtiments au Québec (REIBH 2024, BNQ 3009-500, CNB 2020).',
            'Analyse l\'image fournie et identifie tous les défauts visibles dans la section "' + (sectionName || 'Général') + '".',
            'Réponds UNIQUEMENT avec un objet JSON valide, sans texte avant ni après, sans bloc de code markdown.',
            'Format requis :',
            '{',
            '  "defauts": [',
            '    { "element": "nom_element", "defaut": "description_defaut", "gravite": "URGENT|IMPORTANT|ENTRETIEN" }',
            '  ],',
            '  "observations_generales": "résumé en une phrase"',
            '}',
            'Si aucun défaut n\'est visible, retourne "defauts": [].'
        ].join('\n');

        const userContent1 = {
            text: 'Section inspectée : ' + (sectionName || 'Général') + '. Analyse l\'image et identifie tous les défauts visibles.',
        ...
        let agent1Result;
        try {
            const raw1 = await AIAgents._callAI(systemAgent1, userContent1, 'agent1');
            agent1Result = AIAgents._extractJSON(raw1);
        } catch(err) {
            progress('erreur', 'Agent 1 — Erreur : ' + err.message);
            throw err;
        }

        const defauts = agent1Result.defauts || [];
        progress('agent1_done', 'Agent 1 terminé — ' + defauts.length + ' défaut(s) détecté(s).');

        // Si aucun défaut détecté, on court-circuite le pipeline
        if (defauts.length === 0) {
            progress('pipeline_done', 'Aucun défaut détecté. Pipeline terminé.');
            return {
                texteRapport: 'À la date de l\'inspection, l\'examen visuel de la section « ' +
                              (sectionName || 'Général') + ' » n\'a révélé aucun défaut apparent. ' +
                              'Les composantes observées semblaient en bon état de conservation et conformes ' +
                              'aux normes applicables au moment de l\'inspection.',
                defauts: [],
                evaluations: [],
                niveauGlobal: 'CONFORME',
                recommandationPrincipale: 'Aucune action corrective requise.'
            };
        }

        // ──────────────────────────────────────────────────────────
        // AGENT 2 — Expert REIBH 2024
        // ──────────────────────────────────────────────────────────
        progress('agent2', 'Agent 2 en cours — Évaluation normative REIBH 2024 / BNQ 3009-500 / CNB 2020...');

        const systemAgent2 = [
            'Tu es un expert en réglementation du bâtiment au Québec, spécialiste REIBH 2024, BNQ 3009-500 et CNB 2020.',
            'Évalue la conformité des défauts transmis par l\'Agent 1 et fournis une analyse normative complète.',
            'Réponds UNIQUEMENT avec un objet JSON valide, sans texte avant ni après, sans bloc de code markdown.',
            'Format requis :',
            '{',
            '  "evaluations": [',
            '    {',
            '      "element": "nom_element",',
            '      "norme": "référence normative ex: CNB 9.27.2 / REIBH 2024 art. 12",',
            '      "urgence": "URGENT|IMPORTANT|ENTRETIEN",',
            '      "action_requise": "description action corrective concrète",',
            '      "impact": "conséquence si non traité"',
            '    }',
            '  ],',
            '  "niveau_global": "CRITIQUE|MAJEUR|MINEUR|CONFORME",',
            '  "recommandation_principale": "une phrase de recommandation principale à l\'acheteur"',
            '}'
        ].join('\n');

        const userContent2 = JSON.stringify({
            section: sectionName || 'Général',
            defauts_detectes: defauts,
            observations_generales: agent1Result.observations_generales || ''
        });

        let agent2Result;
        try {
            const raw2 = await AIAgents._callAI(systemAgent2, userContent2, 'agent2');
            agent2Result = AIAgents._extractJSON(raw2);
        } catch(err) {
            progress('erreur', 'Agent 2 — Erreur : ' + err.message);
            throw err;
        }
        ...
        const systemAgent3 = [
            ...
            'Le texte doit être prêt à copier-coller directement dans un rapport officiel d\'inspection.'
        ].join('\n');

        const userContent3 = JSON.stringify({
            section: sectionName || 'Général',
            observations_generales: agent1Result.observations_generales || '',
            defauts: defauts,
            evaluations: agent2Result.evaluations || [],
            niveau_global: agent2Result.niveau_global || '',
            recommandation_principale: agent2Result.recommandation_principale || ''
        });

        let texteRapport;
        try {
            const raw3 = await AIAgents._callAI(systemAgent3, userContent3, 'agent3');
            // Sécurité : échapper tout HTML du LLM avant usage innerHTML
            if (typeof document !== 'undefined' && document.createElement) {
                const escDiv = document.createElement('div');
                escDiv.textContent = raw3;
                texteRapport = escDiv.innerHTML;
            } else {
                // Fallback (environnement sans DOM)
                texteRapport = raw3
                    .replace(/&/g, '&amp;')
                    .replace(/</g, '&lt;')
                    .replace(/>/g, '&gt;');
            ...
            progress('erreur', 'Agent 3 — Erreur : ' + err.message);
            throw err;
        }

        progress('agent3_done', 'Agent 3 terminé — Texte de rapport rédigé.');
        progress('pipeline_done', '✅ Pipeline terminé — Rapport généré avec succès.');
